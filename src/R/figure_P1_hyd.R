...
postscript("figures/Figure3/figure_P1_hyd.ps", width=2, height=2, horizontal = FALSE,  onefile = FALSE, paper = "special") #_normalize
par(mfrow=c(1,1))
par(mar=c(4,4,2,1))  
...
b <- barplot(t(mutations_surface[c(1,3,4),2:3]), beside=T, border=F, ylim=c(0, 20), space=c(0.5, 0.1, 0.5, 0.1, 0.5, 0.1), axes=F, xlab="", ylab="", col=c("#29ABE2", 2))
...
text(colMeans(b)[2], 16, paste("buried p =", sprintf("%11.2e",buried_pval) , sep=' '), cex=0.6)
text(colMeans(b)[2], 12, paste("int p =", sprintf("%11.2e",interface_pval) , sep=' '), cex=0.6)
