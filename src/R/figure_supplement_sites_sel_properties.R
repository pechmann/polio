postscript("figures/Supplement/figure_supplement_sites_sel_properties.ps", width=4, height=4, paper="special", horizontal=F)
...
boxplot(rand[,3], stabwt[sel.wt.stab,6], stabga[sel.ga.stab,6], col=c("#999999", "#29ABE2","red"), ylim=c(-50, 300), xlab="", ylab="", axes=F,  boxwex=0.4, at=c(0.2, 0.8, 1.3), xlim=c(0, 1.5), range=0.5 )
axis(2, at=c(-50, 0, 100, 200, 300), cex.axis=1)
mtext(expression(paste(Delta, "AP")), side=2, line=2.5)
pval_agg  <- wilcox.test(stabwt[sel.wt.stab,6], stabga[sel.ga.stab,6])$p.value
text(0.2, 200, paste("p =", sprintf("%11.2e",pval_agg) , sep=' '), cex=0.6)
