postscript("figures/Figure3/figure_sites_stab_nothyd.ps", width=2.8, height=4, horizontal = FALSE,  onefile = FALSE, paper = "special")
par(mfrow=c(1,1))
par(mar=c(6,6,3,1))
...
sig_positions <- function(d){
  ...
  pos <- sum(d[s,2] > 1 & d[s,4] > 1, na.rm=T)
  neg <- sum(d[s,2] < 1 & d[s,4] > 1, na.rm=T)
...
b <-barplot(cbind(sig_positions(data[1:880,]), sig_positions(data[1566:1747,]), sig_positions(data[1748:2200,]))+1, space=c(0.2, 0.1, 0.8, 0.1, 0.2, 0.1), beside=T, col=c("#29ABE2", 2), border=F, axes=F, ylim=c(0, 50))
...
mtext("Destab mutations", side=3, line=1, cex=0.75)
mtext("no reduct in hyd in GA", side=3, line=0, cex=0.7)
