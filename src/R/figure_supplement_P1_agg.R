...
postscript("figures/Supplement/figure_supplement_P1_agg.ps", width=2, height=2, horizontal = FALSE,  onefile = FALSE, paper = "special") #_normalize
par(mfrow=c(1,1))
par(mar=c(4,4,2,1))  
...
# pseudo-count of 1 added for making low counts visible in the plot!
b <- barplot(t(mutations_surface[c(1,3,4),2:3])+1, beside=T, border=F, space=c(0.5, 0.1, 0.5, 0.1, 0.5, 0.1), ylim=c(0, 20), axes=F, xlab="", ylab="", col=c("#29ABE2", 2))
axis(1, at=c(colMeans(b)), labels=c("b", "i", "s"), cex.axis=0.75, las=1, tick=F, line=0)
axis(2, at=c(0, 10, 20), cex.axis=0.75)
...
text(colMeans(b)[2], 19, paste("buried p =", sprintf("%11.2e",buried_pval) , sep=' '), cex=0.6)
text(colMeans(b)[2], 17, paste("int p =", sprintf("%11.2e",interface_pval) , sep=' '), cex=0.6)
text(colMeans(b)[2], 15, paste("surf p =", sprintf("%11.2e",surface_pval) , sep=' '), cex=0.6)
